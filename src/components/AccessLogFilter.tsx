...
export function AccessLogFilter({ initialLogs }: AccessLogFilterProps) {
  const [filters, setFilters] = useState(createInitialAccessLogFilterState());
  ...
  const handleReset = () => setFilters(createInitialAccessLogFilterState());
  ...
  return (
    <section className="aw-widget" aria-labelledby="access-log-filter-heading">
      ...
      <p className="mt-1 text-xs opacity-60">
        최소 위험도 필터, 복합 조건 일부, 새로고침 후 필터 유지는 아직 준비 중입니다.
      </p>
