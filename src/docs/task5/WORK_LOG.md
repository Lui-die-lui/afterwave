# 과제 5 작업 기록

## 공통 설정

* 프로젝트: Afterwave
* 저장소: https://github.com/Lui-die-lui/afterwave
* 작업 브랜치: `assignment-5`
* 공통 시작 버전: `task5-start`
* 공통 시간 상한: AI별 45분
* 공통 요청 수 상한: AI별 8회
* 요청 수 정의: 사용자가 AI에게 전송한 메시지 수
* 고정 검사 수: 10개
* 고정 검사 문서: `docs/task5/FIXED_TESTS.md`

---

## AI A 시작

* 사용 서비스: Claude Code
* 공개 비교표 표기: 도구 A
* 시작 시각: (사용자가 실제 최초 요청 전송 시각을 KST로 직접 기록)
* 시작 버전: `task5-start`
* 시작 소스: https://github.com/Lui-die-lui/afterwave/tree/task5-start
* 제공한 자료:

  * 현재 저장소
  * `INITIAL_REQUEST.md`
  * `FIXED_TESTS.md`
  * `USAGE_LIMITS.md`
* 이전 작업 대화 제공: 없음

## AI A 종료·인수인계

* 종료 시각: 2026-09-16 20:04 KST 
* 실제 작업시간: 약 19분 (task5 문서가 실제 내용으로 채워진 두 번째 지시 시각 2026-09-16 19:45 KST ~ 종료 20:04 KST 기준 추정치. 첫 번째 지시 메시지의 정확한 전송 시각은 AI가 기록하지 못했으므로, 이보다 더 이르다면 사용자가 실제 값으로 교체할 것)
* 실제 요청 수: 5회 (① 최초 지시 ② AskUserQuestion 응답 "제가 합리적으로 구성" ③ AskUserQuestion 응답 "잘못된 저장소/브랜치입니다 — 중단" ④ 문서 채운 뒤 재전송된 지시 ⑤ 본 WORK_LOG 기록 요청. 도구 호출·파일 읽기는 미포함)
* 검사 실행 회차: 2회 (구현 전 1회, 구현 후 1회)
* FAIL이 포함된 실행 회차: 2회 (1회차: 10개 전체 FAIL — 화면 미구현. 2회차: FT-06, FT-10 FAIL)
* 최종 통과 수: 8/10 (PASS: FT-01, FT-02, FT-03, FT-04, FT-05, FT-07, FT-08, FT-09)
* 첫 실패 검사: 1회차(구현 전) 기준 FT-01부터 전체 실행 불가(FAIL). 2회차(구현 후) 기준 FT-06.
* 종료 버전: `task5-ai-a-handoff`
* 인수인계 문서: `HANDOFF.md`
* 인수인계 누락 여부: 없음
* 남은 문제: 최소 위험도 필터(FT-06) 미구현, 새로고침 후 필터 상태 유지(FT-10) 미구현. 상세는 `HANDOFF.md` 5·6절 참고.

---

## 새 폴더 재현

* 확인 시각:
* 확인 버전: `task5-ai-a-handoff`
* 새 폴더에서 저장소 복제: 성공 / 실패
* 의존성 설치: 성공 / 실패
* 실행 명령 재현: 성공 / 실패
* 고정 검사 실행: 성공 / 실패
* 문서 버전과 실제 버전 일치: 성공 / 실패
* 환경값 누락: 없음 / 누락 내용 기록

---

## AI B 시작

* 사용 서비스: Codex
* 공개 비교표 표기: 도구 B
* 시작 시각:
* 시작 버전: `task5-ai-a-handoff`
* 제공한 자료:

  * 현재 저장소
  * `HANDOFF.md`
  * 저장소 안의 과제 5 문서
* AI A 대화 전문 제공: 없음
* AI A가 남긴 문서와 AI B가 받은 문서의 동일성: 일치 / 불일치
* 인수인계 누락 여부:

## AI B 종료

* 종료 시각:
* 실제 작업시간:
* 실제 요청 수:
* 검사 실행 회차:
* FAIL이 포함된 실행 회차:
* 최종 통과 검사:
* 검사 삭제: 0건
* 검사 완화: 0건
* 기대값 변경: 0건
* 완료한 기능:
* 종료 버전: `task5-complete`

---

## 전체 작업 순서

1. AI A 시작:
2. AI A 종료·인수인계:
3. AI B 시작:
4. AI B 종료:

---

## 이름을 가린 비교 결과

| 측정 항목    |                 도구 A |                 도구 B |
| -------- | -------------------: | -------------------: |
| 실제 작업시간  |                      |                      |
| 실제 요청 수  |                      |                      |
| 검사 실행 회차 |                      |                      |
| 오류 회차    |                      |                      |
| 최종 통과 수  |                      |                      |
| 시작 버전    |        `task5-start` | `task5-ai-a-handoff` |
| 종료 버전    | `task5-ai-a-handoff` |     `task5-complete` |

## 다음 작업의 도구 선택 기준

구조 파악과 초기 구현 속도가 중요한 작업에는 도구 A를 먼저 사용하고, 저장소와 인수인계 문서만으로 재현하고 검증해야 하는 작업에는 도구 B를 우선 사용한다.
